...
void clrscfifo()        //For fifo implementation this function will display the final results
{
    if(strcmp(status,"HIT")==0)
    {
	     ...
	     drawText("FIRST IN FIRST OUT ALGORITHM!!",-500,800,0);
    ...
    else
    {
         ...
         glClear(GL_COLOR_BUFFER_BIT);
         glColor3f(0.99,0.94,0.6);
	     drawText("FIRST IN FIRST OUT ALGORITHM!!",70,1150,0);
         glColor3f(0.75,0.7,1);
...
int fifo(int pages[9],int cpg)               //returns position to fill page in
{
    ...
    if(b == 0)  //page fault
    {
        //return the position of the box to be filled in.
        return (pgf++)%3;
    }
}
...
void drawstatusfifo()                                               //draws status of each page
{
    glColor3f(0.99,0.94,0.6);
	drawText("FIRST IN FIRST OUT ALGORITHM!!",60,880,0);
    glColor3f(1,0,1);
...
void drawstatuslru()                                               //draws status of each page
{
    glColor3f(0.99,0.94,0.6);
...
void dispfifo()
{
     ...
     boxpush(pages[curpage], 1);                           //keep pushing till reach end
     glPopMatrix();
     glutSwapBuffers();
     glFlush();
}
...
void displru()
{
     ...
     glPushMatrix();
     glTranslatef(diffx,diffy,0);                       //push box down/left
     boxpush(pages[curpage], 0);                           //keep pushing till reach end
     glPopMatrix();
     glutSwapBuffers();
     glFlush();
}
...
int main(int argc,char **argv)
{
    ...
    printf("Press 1 for FIFO Implementation and 2 for LRU Implementation.\n");
    scanf("%d",&option);
    ...
    if (option ==1){
        ...
        window_id=glutCreateWindow("FIRST IN FIRST OUT ALGORITHM");
        glutKeyboardFunc(mykey);
        init();
        glutDisplayFunc(dispfifo);
